...
# commit_hunter.sh
#
# This script reads two build‐output strings (GOOD_BUILD and BAD_BUILD),
# extracts the OpenJ9, OMR, and JCL commit hashes, then prints the three
# “compare” URLs for you.
...
# Helper: extract the first matching hex‐hash after a given prefix
extract_hash() {
  local prefix="$1"   # e.g. "OpenJ9"
  local text="$2"     # the multi‐line build string
  printf '%s\n' "$text" \
    | grep -Po "${prefix}\s*-\s*\K[0-9a-f]+" \
    | head -n 1
}

# 1) Extract “good” SHA values:
...
if [[ -z "$good_openj9" || -z "$good_omr" || -z "$good_jcl" ]]; then
  echo "ERROR: couldn’t parse one or more hashes from GOOD_BUILD"
  exit 1
fi

# 2) Extract “bad” SHA values:
...
if [[ -z "$bad_openj9" || -z "$bad_omr" || -z "$bad_jcl" ]]; then
  echo "ERROR: couldn’t parse one or more hashes from BAD_BUILD"
  exit 1
fi

# 3) Construct the three compare URLs:
